...
export default function Winners() {
  ...
  // Group by each award tag — a project with multiple awards appears in each category
  const byTag = {};
  for (const p of projects) {
    const tags = p.award_tags?.length ? p.award_tags : (p.award_tag ? [p.award_tag] : ['Other']);
  ...
  const totalAwards = projects.reduce((n, p) => n + (p.award_tags?.length || 1), 0);

  return (
    <div className="stack">
      <h1 style={{ marginBottom: 4 }}>🏆 Winners</h1>
      <p className="muted" style={{ marginBottom: 8 }}>{totalAwards} award{totalAwards !== 1 ? 's' : ''} across {projects.length} project{projects.length !== 1 ? 's' : ''}</p>
            ...
            {byTag[tag].map((p) => (
              <div key={p.id} style={{ padding: '14px 16px', background: 'var(--surface-2)', borderRadius: 8, borderLeft: '4px solid var(--accent)' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap', marginBottom: 4 }}>
                  ...
                  {p.award_tags?.length > 1 && (
                    <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                      {p.award_tags.map((t) => (
                        <span key={t} style={{ fontSize: 11, padding: '2px 7px', borderRadius: 4, background: 'var(--accent)', color: '#fff', fontWeight: 600 }}>{t}</span>
                      ))}
                    </div>
                  )}
                </div>
                ...
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 4 }}>
                  {p.demo_video_link && (
                    <a href={p.demo_video_link} target="_blank" rel="noopener noreferrer" style={{ fontSize: 13 }}>▶ Watch demo ↗</a>
                  )}
                  {p.git_link && (
                    <a href={p.git_link} target="_blank" rel="noopener noreferrer" style={{ fontSize: 13 }}>⌥ GitHub ↗</a>
                  )}
                  {p.app_url && (
                    <a href={p.app_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 13 }}>🌐 Live app ↗</a>
                  )}
                </div>
              </div>
            ))}
